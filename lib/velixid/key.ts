...
export class VelixIDKey {
  public static generate(publicKey: string, cb: Function) {
    var newVelixID = "V";
    // var possible = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
    var possible = "0123456789";

    for (var i = 0; i < 7; i++)
      newVelixID += possible.charAt(
  ...
  public loadKey(cb: Function): any {
    ...
    this.docClient.query(query, (err, doc) => {
      if (err) {
        cb(err, doc);
      } else if (doc.Items) {
        console.log('Item', doc.Items);
  ...
  public newAuthRequest(
    ...
    this.docClient.put(params, (err, doc) => {
      if (err) {
        throw "ERROR: " + err.message;
      } else {
        cb(null, request);
